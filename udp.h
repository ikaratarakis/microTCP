...
#define MICROTCP_MSS 1460 //Maximum segment size
#define MICROTCP_RECVBUF_LEN 20000
...
typedef struct
{
  ...
  int type;                     /**< The client(0) or server(1) */
...
typedef struct
{
  ...
  struct timespec timestamp;           /**< 32-bits for future use */
  uint32_t meas_delay;          /**< 32-bits for future use */
  uint32_t future_use2;         /**< 32-bits for future use */
  uint32_t checksum;            /**< CRC-32 checksum, see crc32() in utils folder */
} microtcp_header_t;

static volatile struct timespec st_owd;
static volatile uint32_t m_delay;
...
void setStart(struct timespec start);

struct timespec getStart(void);

void setMeas_Delay(uint32_t md);

uint32_t getMeas_Delay(void);
